...
public class DynamicByteBuffer implements Comparable<ByteBuffer>
{
    ...
    private Function<Integer,Float> expandFactor;

    public DynamicByteBuffer (int initialCapacity, Function<Integer, Float> expandFactor)
    {
        if(expandFactor.apply(initialCapacity) < 1)
        {
    ...
    {
        this(initialCapacity, (capacity) -> 2.0f);
    }
    ...
    {
        ensureSpace(4);
        System.out.print("putInt -> (" + value + ") " + (char) value + "\n");
    ...
    public void setExpandFactor (Function<Integer, Float> expandFactor)
    {
        if(expandFactor.apply(capacity()) < 1)
        {
            throw new IllegalArgumentException(
                    "The expand factor must be greater or equal to 1!");
        }
    ...
    {
        float factor = expandFactor.apply(capacity());
        return factor < 1 ? 1.0f : factor;
    }
    ...
    private void ensureSpace (int needed)
    {
        ...
        int newCapacity = (int) (byteBuffer.capacity() * getExpandFactor());
        while(newCapacity < (byteBuffer.capacity() + needed))
        {
            newCapacity *= getExpandFactor();
        }
